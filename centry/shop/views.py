...
class OrderView(View):
    def get(self, request):
        ...
        orders = Order.objects.filter(
            customer=customer_id).order_by("-date").order_by("-id")
        print(orders)
...
class Signup(View):
    ...
    def validateData(self, userData):
        ...
        if not userData['name'] or not userData['email'] or not userData['phone'] or not userData['password'] or not userData['confirm_password']:
            error["field_error"] = "VOUS DEVEZ ECRIRE SUR TOUT LES CHAMPS"
        elif len(userData['password']) < 8 and len(userData['confirm_password']) < 8:
            error['minPass_error'] = "POUR DE MESURE DE SECURITE LE MDP DOIT AVOIR  PLUS DE 8 char"
        elif len(userData['name']) > 25 or len(userData['name']) < 3:
            error["name_error"] = "LE NOM DOIT AVOIR PLUS DE  3-25 charecters"
        elif len(userData['phone']) != 10:
            error["phoneNumber_error"] = "le num doit voir  10 charecters."
        elif userData['password'] != userData['confirm_password']:
            error["notMatch_error"] = "le mot de pass ne correspond pas "
